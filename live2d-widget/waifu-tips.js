...
function loadWidget(config) {
	...
	(function registerEventListener() {
		...
		document.querySelector("#waifu-tool .fa-camera-retro").addEventListener("click", () => {
			showMessage("照好了嘛，小刘我是不是超可爱呢？", 6000, 9);
		...
		window.addEventListener("visibilitychange", () => {
			if (!document.hidden) showMessage("哇，小刘你终于回来了～", 6000, 9);
		});
	})();
	...
	(function welcomeMessage() {
		...
		if (location.pathname === "/") { // 如果是主页
			...
			if (now > 5 && now <= 7) text = "小刘早上好！一日之计在于晨，美好的一天就要开始了，记得喝牛奶呀。";
			else if (now > 7 && now <= 11) text = "小刘上午好！工作顺利嘛，不要久坐，多起来走动走动哦！";
			else if (now > 11 && now <= 13) text = "小刘中午了，工作了一个上午，现在是午餐时间！";
